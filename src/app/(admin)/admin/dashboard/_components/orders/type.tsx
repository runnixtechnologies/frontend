...
import { useGetTopOrdersByTypeQuery } from "@/lib/redux/api/dashboard"
...
export default function TopOrdersByTypeCard() {
  const { data, isLoading, isError, refetch } = useGetTopOrdersByTypeQuery()

  // Map API -> chart rows + config + total (memoized)
  const { chartData, chartConfig, total } = useMemo(() => {
    // Slice transform should already map to [{ key, total }]; if not, adapt here:
    // raw payload: { name, total_orders }
    const source = (data ?? []) as Array<
      | { key?: string; total?: number }
      | { name?: string; total_orders?: number }
    >

    const rows: Row[] = source.map((x, i) => {
      const name = (x as any).key ?? (x as any).name ?? "Unknown"
    ...
    rows.forEach((r, i) => (cfg[`t_${i}`] = { label: r.name, color: r.color }))

    return { chartData: rows, chartConfig: cfg, total }
  }, [data])
  ...
  const renderCustomizedLabel = ({
    ...
    const fill = isColorDark(chartData[index].color) ? "#FFFFFF" : "#232323"
