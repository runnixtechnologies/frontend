// app/(your-route)/_components/standard.tsx
"use client"
...
/* only validate known mapped fields – values come in as strings from RHF */
...
export default function StandardDelivery() {
  const kind = "standard-delivery" as const

  const { data, isLoading, isError } = useGetAllPricingQuery({ type: kind })

  // stabilize items for downstream hooks
  ...
  // keep defaults in sync with server
  useEffect(() => {
    form.reset(currentValues)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    currentValues.base,
    currentValues.kmPrice,
    currentValues.minPrice,
    currentValues.waitingTime,
  ])
  ...
  return (
    <div className="w-full pr-6 space-y-[36px]">
      ...
      {/* Known fields first, then any extra dynamic entries from the API */}
