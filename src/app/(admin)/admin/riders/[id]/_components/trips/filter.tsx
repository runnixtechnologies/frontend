...
interface TripsFiltersProps {
  onFilterChange?: (filters: TripsFilterValues) => void
}

export function TripsFilters({ onFilterChange }: TripsFiltersProps) {
  const [filters, setFilters] = useState<TripsFilterValues>({
    type: "all-type",
  ...
  const updateActiveFiltersCount = (currentFilters: TripsFilterValues) => {
    let count = 0
