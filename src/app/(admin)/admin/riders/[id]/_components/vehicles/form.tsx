...
export const profileSchema = z.object({
  ...
  manufacturer: z.string().min(1, "Last name is required"),
...
export default function VehicleInformation() {
  ...
  const [vehicleData, setVehicleData] = useState<ProfileFormData>(initialData)
  ...
  const handleEdit = (field: EditableField) => {
    ...
    form.setValue(field, vehicleData[field])
  }
  ...
  const handleSave = async (field: EditableField) => {
    ...
    if (isValid) {
      ...
      setVehicleData((prev) => ({ ...prev, [field]: newValue }))
  ...
  const handleCancel = () => {
    if (editingField) {
      form.setValue(editingField, vehicleData[editingField])
    }
    setEditingField(null)
  }
  ...
  return (
    <div className="w-full pr-6 space-y-[48px]">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {renderField("year", "Vehicle Year", vehicleData.year)}
        {renderField(
          "manufacturer",
          "Vehicle Manufacturer",
          vehicleData.manufacturer
        )}
        {renderField("model", "Vehicle Model", vehicleData.model)}
        {renderField("license", "Vehicle License Plate", vehicleData.license)}
      </div>

      <div className="w-full">
        {renderField("color", "Vehicle Colour", vehicleData.color)}
      </div>
    </div>
  )
}
