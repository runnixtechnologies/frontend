...
export type MerchantDetailFilterValues = {
  type: string
  location: string
  status: string[]
  dateRange: string
...
interface DeliveryFiltersProps {
  onFilterChange?: (filters: MerchantDetailFilterValues) => void
}

export function DeliveryFilters({ onFilterChange }: DeliveryFiltersProps) {
  const [filters, setFilters] = useState<MerchantDetailFilterValues>({
    type: "all-type",
  ...
  const updateActiveFiltersCount = (
    currentFilters: MerchantDetailFilterValues
  ) => {
    let count = 0
