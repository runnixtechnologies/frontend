...
import { UserFilters, type UserFilterValues } from "./_components/filters"
import { UserStats } from "./_components/stats"
import { UsersTable } from "./_components/table"
import UserTabs, { TabKey } from "./_components/tabs"
...
export default function RidersPage() {
  ...
  const [filtered, setFiltered] = useState<User[]>(usersData)
  ...
  const tabCounts = useMemo<Record<TabKey, number>>(
    () => ({
      ...
      active: usersData.filter((d) => d.status === "Active").length,
      inactive: usersData.filter((d) => d.status === "Inactive").length,
      suspended: usersData.filter((d) => d.status === "Suspended").length,
    }),
    []
  )
  ...
  useEffect(() => {
    ...
    if (activeTab !== "all") {
      const statusLabel = (activeTab.charAt(0).toUpperCase() +
        activeTab.slice(1)) as User["status"]
  ...
  const handleFilterChange = (newFilters: Partial<UserFilterValues>) => {
    setFilters((prev) => ({
      ...prev,
      ...newFilters,
    }))
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6 p-4 md:p-6">
        ...
        <div className="w-full bg-white rounded-lg border flex flex-col gap-3">
          ...
          <UsersTable filters={filters} data={filtered} />
        </div>
      </div>
    </DashboardLayout>
  )
